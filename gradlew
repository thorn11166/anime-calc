...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    if [ -n "$ROOTDIRSRAW" ] ; then
        ROOTDIRS=`echo "$ROOTDIRSRAW" | awk -F/ '{s=1; for (i=1; i<=NF; i++) {if ($i ~ /^[a-zA-Z]:$/) {s=0; p=$i; next} if (s==1) p=p"/"$i; else p=$i":"p; s=1} print p}'`
    fi
    if [ -n "$ROOTDIRS" ] ; then
        GRADLE_OPTS="$GRADLE_OPTS \"-Dorg.gradle.appname=$APP_BASE_NAME\" \"-Dorg.gradle.appname=$APP_BASE_NAME\" \"-Xmx64m\" \"-Xms64m\""
    fi
fi
